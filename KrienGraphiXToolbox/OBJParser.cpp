...
HRESULT OBJParser::parseFile( _In_ const std::string &fileName, _In_ const std::string &fileDir, _Inout_opt_ KgmData *outputData )
{
	...
	if ( objData.mtllib.size() > 0 && outputData != NULL )
	{
		...
		if ( !parseMtl( ssMtl.str(), outputData->mats ) )
		{
	...
	std::map< std::string, std::vector<MeshData> > sortedModels;
	std::vector<MeshData*> tempModels;
	...
	for ( i = objData.faces.cbegin(); i != objData.faces.cend(); ++i )
	{
		...
		if ( i->groupName.size() > 0 || i->usemtl.size() > 0 )
		{
			...
			MeshData newModel( i->groupName, i->usemtl, int(outputData->indices.size()) );
			storeMeshData( i->groupName, newModel, &sortedModels, &tempModels );
		}
	...
	{
		MeshData newModel( 0, int(outputData->indices.size()) );
		storeMeshData( "", newModel, &sortedModels, &tempModels );
	}
	...
	std::map< std::string, std::vector<MeshData> >::iterator mIt;
		...
		if ( mIt->first.size() > 0 )
		{
			...
			} else																	//make sure there are no duplicate object names
			{
				...
				for ( i = mIt->second.begin(); i != mIt->second.end(); ++i )
				{
					...
					ssName << i->meshName << '_' << nameCount;
					nameCount++;

					i->meshName = ssName.str();
					outputData->models.push_back(*i);
				}
			}
		} else																		//give names to unnamed objects
		{
			UINT unnamedCount = 1;
			for ( i = mIt->second.begin(); i != mIt->second.end(); ++i )
			{
...
bool OBJParser::parseMtl( const std::string &input, std::vector<MatData> &mtlData )
{
	struct mtlGram : qi::grammar<std::string::const_iterator, std::vector<MatData>(), Skipper>
	{
		mtlGram() : mtlGram::base_type(start)
		{
			skipLine = (lit('#') | lit("illum")) >> qi::skip(qi::blank)[ *qi::print ];

			specPwr = "Ns" >> double_[ phx::bind(&MatData::specPwr, _r1) = qi::_1 ];
			optDens = "Ni" >> double_[ phx::bind(&MatData::optDens, _r1) = qi::_1 ];
			transD  = 'd' >> double_[ phx::bind(&MatData::trans, _r1) = qi::_1 ];
			transTr = "Tr" >> double_[ phx::bind(&MatData::trans, _r1) = 1.0f - qi::_1 ];

			auto addNewmtl   = phx::push_back( phx::bind(&MatData::name, _r1), qi::_1 );
			auto addAmbMap   = phx::push_back( phx::bind(&MatData::ambMap, _r1), qi::_1 );
			auto addDiffMap  = phx::push_back( phx::bind(&MatData::diffMap, _r1), qi::_1 );
			auto addSpecMap  = phx::push_back( phx::bind(&MatData::specMap, _r1), qi::_1 );
			auto addTransMap = phx::push_back( phx::bind(&MatData::transMap, _r1), qi::_1 );
			auto addBumpMap  = phx::push_back( phx::bind(&MatData::bumpMap, _r1), qi::_1 );
			...
			bumpMap  = (lit("map_bump") | lit("bump")) >> eps[phx::resize( phx::bind(&MatData::bumpMap, _r1), 0 )]
							>> lexeme[*(print - iso8859::space)[addBumpMap]] >> qi::skip(qi::blank)[ *qi::print ];

			transFilter = "Tf" >> repeat(3)[ double_[phx::push_back( phx::bind(&MatData::transFilter, _r1), qi::_1 )] ];
			ambClr      = "Ka" >> repeat(3)[ double_[phx::push_back( phx::bind(&MatData::ambClr, _r1), qi::_1 )] ];
			diffClr     = "Kd" >> repeat(3)[ double_[phx::push_back( phx::bind(&MatData::diffClr, _r1), qi::_1 )] ];
			specClr     = "Ks" >> repeat(3)[ double_[phx::push_back( phx::bind(&MatData::specClr, _r1), qi::_1 )] ];
			emmisClr    = "Ke" >> repeat(3)[ double_[phx::push_back( phx::bind(&MatData::emmisClr, _r1), qi::_1 )] ];
		...
		qi::rule<std::string::const_iterator, std::vector<MatData>(), Skipper> start;
		qi::rule<std::string::const_iterator, MatData(), Skipper> material;
		qi::rule<std::string::const_iterator, void(MatData&), Skipper> specPwr, optDens, transD, transTr;
		qi::rule<std::string::const_iterator, void(MatData&), Skipper> transFilter, ambClr, diffClr, specClr, emmisClr;
		qi::rule<std::string::const_iterator, void(MatData&), Skipper> newmtl, ambMap, diffMap, specMap, transMap;
		qi::rule<std::string::const_iterator, void(MatData&), Skipper> bumpMap;
		Skipper skipLine;
	} mtlGrammar;
 ...
 */
void OBJParser::storeMeshData( std::string modelName, MeshData &data, 
									std::map< std::string, std::vector<MeshData> > *sortedModels, std::vector<MeshData*> *tempModels )
{
	typedef std::map< std::string, std::vector<MeshData> >::iterator modelIt;
	modelIt mIt;
	mIt = sortedModels->find(modelName);

	if ( mIt == sortedModels->end() )
	{
		std::pair<modelIt, bool> res = sortedModels->insert( std::pair< std::string, std::vector<MeshData> >(modelName, std::vector<MeshData>(1, data)) );
		mIt = res.first;
	} else
	{
